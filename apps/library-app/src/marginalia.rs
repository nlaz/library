...
pub(crate) async fn list_cards(state: State<'_, AppState>) -> Result<Vec<CardRec>, String> {
    ...
    tauri::async_runtime::spawn_blocking(move || notes::load_cards(&ctx))
        .await
...
pub(crate) async fn create_card(
    ...
    tauri::async_runtime::spawn_blocking(move || {
        ...
        notes::create_card(&mut lib, &ctx, input, &embed).map_err(|e| e.to_string())
    })
    .await
    .map_err(|e| e.to_string())?
}
...
pub(crate) async fn update_card(
    ...
    tauri::async_runtime::spawn_blocking(move || {
        let mut lib = eng.lib.write().expect("library lock poisoned");
        notes::update_card(&mut lib, &ctx, card, &embed).map_err(|e| e.to_string())
    })
    .await
    .map_err(|e| e.to_string())?
}
